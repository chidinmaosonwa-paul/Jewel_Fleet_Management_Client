...
const Journeys = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.response?.data?.error?.[0] || 'Failed to save journey');
    }
  };
  ...
  return (
    <div>
      <div className="page-header">
        <h1>Journeys</h1>
        <button className="btn-primary" onClick={() => { setShowForm(!showForm); setEditId(null); setFormData({ vehicleId: '', destinationId: '', departureTime: '' }); }}>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <select name="destinationId" value={formData.destinationId} onChange={handleChange} required>
                ...
                {destinations.map((d) => (
                  <option key={d._id} value={d._id}>{d.name} — ₦{d.baseFare.toLocaleString()}</option>
                ))}
              </select>
            </div>
          </div>
              ...
              <tr key={journey._id}>
                ...
                <td>
                  <select
                    ...
                    onChange={(e) => handleStatusChange(journey._id, e.target.value)}
                  >
                ...
                <td>
                  <button className="btn-edit" onClick={() => handleEdit(journey)}>Edit</button>
                  <button className="btn-delete" onClick={() => handleDelete(journey._id)}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
